...
func (d *DB) UpsertHubIdentity(identity HubIdentity) (*HubIdentity, error) {
	...
	row := d.db.QueryRow(`
		...
		ON CONFLICT (id) DO UPDATE SET
			...
			-- Never wipe an existing keypair with empty values from a partial upsert.
			public_key = CASE WHEN excluded.public_key = '' THEN hub_identity.public_key ELSE excluded.public_key END,
			private_key = CASE WHEN excluded.private_key = '' THEN hub_identity.private_key ELSE excluded.private_key END,
