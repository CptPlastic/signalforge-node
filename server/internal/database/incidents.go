...
// ListActiveIncidents returns incidents that are currently active or in monitoring status.
func (d *DB) ListActiveIncidents() ([]Incident, error) {
	rows, err := d.db.Query(incidentSelect + ` WHERE status IN ('active', 'monitoring') ORDER BY updated_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
